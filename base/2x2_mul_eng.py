...
class SimpleMultiplicationEnv:
    ...
    def step(self, action):
        ...
        if action == 0 and self.current_step == 0:
            ...
            reward = 0.3
            self.steps.append(f"Ones: {self.number % 10} x {self.multiplier} = {self.ones_result}")
            ...
            reward = 0.3
            self.steps.append(f"Tens: {tens} x {self.multiplier} + {self.carry} = {self.tens_result}")
            ...
            reward = 0.3
            self.steps.append(f"Hundreds: {hundreds} x {self.multiplier} + {self.carry} = {self.hundreds_result}")
            ...
            if None not in [self.ones_result, self.tens_result, self.hundreds_result]:
                ...
                if answer == self.correct_answer:
                    reward = 1.0
                    info['message'] = "Correct answer!"
                else:
                    reward = -0.5
                    info['message'] = f"Wrong! Correct answer: {self.correct_answer}"
            else:
                reward = -1.0
                info['message'] = "Missing steps!"
...
if __name__ == "__main__":
    print("Transformer-Based Math RL Model Training")
    print("---------------------------------------")
